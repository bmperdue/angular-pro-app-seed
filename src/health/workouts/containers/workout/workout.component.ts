...
<div class="workout">
   ...
   <div *ngIf="workout$ | async as workout; else loading; ">
     <workout-form [workout]="workout" (create)="addWorkout($event)" (update)="updateWorkout($event)"  (remove)="removeWorkoutk($event)"></workout-form>
   </div>
   <ng-template #loading>
     <div class="message">
...
export class WorkoutComponent implements OnInit, OnDestroy {
  ...
  async updateWorkout(event: Workout) {
    const key = this.route.snapshot.params.id; // Grab a snapshot of the parms you are on. Done statically
